...
//      Version 		:v2.0
//      IDE                     :MPLAB X IDE v1.51
//      Compiler                :HI-TECH PICC v9.80
//============================================================================================================
...
#define	CHANNEL0	0b10000001	// AN0 ( Ultrasonic LVEZ1 )
#define	CHANNEL1	0b10001001	// AN1 ( Sharp Infrared Distance Sensor )
...
void main(void)
{
    ...
    b_light = 1;    //activate the LCD backlight
...
void init()
{
    ...
    delay_ms(10);
    // ADC configuration
    ADCON1 = 0b10000100;//Configure RA0, RA1 and RA3 as Analog Input, left justified
                        //The rest of AN pin is digital pin

    // setup for capture pwm from ultrasonic LVEZ1 PWM output
   RBIE = 1;		// enable interrupt on change of port B
    ...
    send_config(0b00111000);	//8-bit interface
    ...
    TX_PIN = 1;
    b_light = 0;    //LCD backlight is off initially
...
void line_follow()							
{
	unsigned char memory = 0;   //variable to memory
...
void ultrasonic(void)		
{
    ...
    {
        if(!sw1)//if sw1 is pressed															// if button s1 is pressed
        {
            while(!sw1);    //wait for sw1 to be released													// wait until sw1 is release
            n++;																// increment n
